	...
	protected static class OAuth2Config extends
		...
		public OAuth2Config() throws Exception {
			...
			ClientDetailsService csvc = new InMemoryClientDetailsServiceBuilder()
			        ...
			        // video service
					.withClient("mobile").authorizedGrantTypes("password").secret("secret")
					.authorities("ROLE_CLIENT", "ROLE_TRUSTED_CLIENT")
					.scopes("read", "write").resourceIds("hotmeals/suppliefs")
					.and()
					// Create a second client that only has "read" access to the
					// video service
					.withClient("mobileReader").authorizedGrantTypes("password").secret("secret")
					.authorities("ROLE_CLIENT")
					.scopes("read").resourceIds("hotmeals/suppliefs")
